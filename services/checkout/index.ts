...
import { carts, cartItems, orders, orderItems, payments, products, productVariants, sequelize } from '../../models';
import { paymentQueue, addJob, QUEUE_CONFIG } from '../../config/queue';
import { Transaction, QueryTypes } from 'sequelize';
import { AuthenticatedRequest } from '../../types';
import { Response } from 'express';
import { paginatedResponse } from '../../middlewares/paginate';

const checkout = asyncErrorHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const order = await sequelize.transaction(async (t: Transaction) => {
          ...
          const orderNumber = (orderNumberResult[0] as any).order_number;
...
const getOrder = asyncErrorHandler(async (req: AuthenticatedRequest, res: Response) => {
    const userId = req.user!.id;
...
const getUserOrders = asyncErrorHandler(async (req: AuthenticatedRequest, res: Response) => {
    const userId = req.user!.id;
